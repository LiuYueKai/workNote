...
| ucf-mdf-design | | iuap5-ucf-mdf-design | tqigxqgr | 是 | |
| fc_omc_fe | fin-fe| iuap5-omc-fe| f9mp4vyq| 是 | |
| metadata-front | | iuap5-metadata-fe| n2p1rg9q| 是 | |
...
| ucf-basedoc-fe | [iuap_ap/ucf-basedoc-fe](http://git.yonyou.com/iuap_ap/ucf-basedoc-fe)                 | 公共档案(基础档案)                                     | 本地 |
...
### 本地产出
* 修改nodemodules
	`sed -i '' 's/return hosts\[key\]\[process.env.NODE_ENV\]/const hosts1 = {     api: "",     workbench: "${pom.diwork.login}",     u8c: "${pom.diwork.login}",     yzone: "undefined-yzone",     u8cportal: "undefined-u8cportal",     manageTeamEnter: "undefined-manageTeamEnter",     login: "${sso.url}",     euc: "${user.url}", 		user: "${user.url}",     market: "undefined-market",     upload: "undefined-upload",     info: "undefined-info",     speak: "undefined-speak",     honor: "undefined-honor",     sendHonor: "undefined-sendHonor",     ticket: "undefined-ticket",     order: "undefined-order",     cloundyy: "undefined-cloundyy",     ref: "${ms.url}",     org: "${org.url}",     paramsetting: "undefined-paramsetting",     platform: "undefined-platform",     developerPath: "undefined-developerPath",     orderSearch: "undefined-orderSearch", 		tenantauth: "${pom.org.permission.url}", 		ms: "${ms.url}", 		bd: "${bdyyuap.url}", 		sso: "${sso.url}", 		manager: "${manager.url}", 		isearch: "${config.ucf.search.endpoint}", 		message: "${message.url}", 		fastdfs: "${fastdfs.url}", 		staff: "${staff.url}", 		basedoc: "${bd.url}", 		mc: "${mc.url}", 		bdext: "${bd-ext.url}", 		upc: "${upc.url}", 		auth: "${auth.url}" 	}; 	return hosts1\[key\]/g' node_modules/@diworkfe/public-components/build/utils.js`
* 替换src下的专属化变量
...
### 本地产出
* 修改nodemodules
	`sed -i '' 's/return hosts\[key\]\[process.env.NODE_ENV\]/const hosts1 = {     api: "\/manager",     workbench: "${pom.diwork.login}",     u8c: "${pom.diwork.login}",     yzone: "undefined-yzone",     u8cportal: "undefined-u8cportal",     manageTeamEnter: "undefined-manageTeamEnter",     login: "${sso.url}",     euc: "${user.url}", 		user: "${user.url}",     market: "undefined-market",     upload: "undefined-upload",     info: "undefined-info",     speak: "undefined-speak",     honor: "undefined-honor",     sendHonor: "undefined-sendHonor",     ticket: "undefined-ticket",     order: "undefined-order",     cloundyy: "undefined-cloundyy",     ref: "${ms.url}",     org: "${org.url}",     paramsetting: "undefined-paramsetting",     platform: "undefined-platform",     developerPath: "undefined-developerPath",     orderSearch: "undefined-orderSearch", 		tenantauth: "${pom.org.permission.url}", 		ms: "${ms.url}", 		bd: "${bdyyuap.url}", 		sso: "${sso.url}", 		manager: "${manager.url}", 		isearch: "${config.ucf.search.endpoint}", 		message: "${message.url}", 		fastdfs: "${fastdfs.url}", 		staff: "${staff.url}", 		basedoc: "${bd.url}", 		mc: "${mc.url}", 		bdext: "${bd-ext.url}", 		upc: "${upc.url}", 		auth: "${auth.url}" 	}; 	return hosts1\[key\]/g' node_modules/@diworkfe/public-components/build/utils.js`
* 替换src下的专属化变量
	sed -i ''  "s/window\._IUAPPREMISESFLAG/true/g" `grep window._IUAPPREMISESFLAG -rl src`
* 产出命令
...
## ucf-basedoc-fe
### 本地产出
/src/web/common/config.env.js下将http://ucfbasedoc.test.app.yyuap.com替换为${ucfbasedoc.rest.url}
`npm run build:premises`
...
## bPaaS_dispatch_fe
### 本地产出
* 将dist下的https://u8cmsg-daily.yyuap.com替换为${message.url}
...
sed -i '' 's@3006@${auth.port}@g' pm2.json 
sed -i '' 's@https://u8cms-daily.yyuap.com@${ms.url}@g' static/public/javascripts/index.min.js 
```
...
只需要替换一次
```
sed -i '' 's@http://127.0.0.1:8000@${upcs.url}@g' pm2.json 
sed -i '' 's@3006@${upc.port}@g' pm2.json 
```